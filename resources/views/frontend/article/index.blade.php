    ...
    <div class="col-lg-12 col-md-12 col-sm-12">
        <div class="left_content">
            <div class="single_page">
                ...
                <h1>{{$article->title}}</h1>
                <div class="post_commentbox"><a href="#"><i class="fa fa-user"></i>{{$article->user->name}}</a>
                    <span><i class="fa fa-calendar"></i>{{$article->created_at->format("d.m.Y h:i")}}</span>
                    @if($article->categories)<a href="#"><i
                            class="fa fa-tags"></i>{{$article->categories->first()->name}}</a>@endif
                </div>
                        ...
                        @if ($media->media_types_id == 1)
                            <a class="fancybox" rel="gallery1"
                               ...
                               title="Vertical - Special Edition! (cedarsphoto)">
                                <img
                                    src="{{config("app.url").\Illuminate\Support\Facades\Storage::url("images/small/".$media->src) }}"
                                    alt=""/>
                            </a>
                        @else
